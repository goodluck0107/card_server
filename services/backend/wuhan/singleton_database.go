...
func GetDBMgr() *DBMgr {
	if DBMgrSingleton == nil {
		...
		phpDsn := strings.ReplaceAll(con.DB, "chess", "mahjong")
		xlog.Logger().Warnf("init php dsn: %s", phpDsn)
		DBMgrSingleton.PHPDB, err = gorm.Open("mysql", phpDsn)
		if err != nil {
			xlog.Logger().Errorln(err)
			panic(fmt.Sprintf("init php database error: %s", err.Error()))
		}

		DBMgrSingleton.db_M.LogMode(true)
		DBMgrSingleton.db_M.SetLogger(xlog.DBLogger())

		DBMgrSingleton.PHPDB.LogMode(true)
		DBMgrSingleton.PHPDB.SetLogger(xlog.DBLogger())
