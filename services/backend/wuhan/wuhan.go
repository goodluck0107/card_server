...
func (s *Server) Run(ctx context.Context) {
	...
	for {
		...
		if time.Now().Hour() == 2 {
			...
			xlog.Logger().Warn("ticker run SaveRedisToDBData beg: ", time.Now())
			// redis数据同步写入数据库
			go s.SaveRedisToDBData()
		} else if time.Now().Hour() == 4 {
			// 数据统计
			xlog.Logger().Warn("ticker run WriteStatisticsData beg: ", time.Now())
			go s.WriteStatisticsData()

			xlog.Logger().Warn("ticker run WriteGlodUserStatisticsData beg: ", time.Now())
			go s.WriteGlodUserStatisticsData()

			xlog.Logger().Warn("ticker run DeleteOldData beg: ", time.Now())
			go s.DeleteOldData()
		}
	}
}
...
func (s *Server) DeleteOldData() {
	timeLimit := time.Now().AddDate(0, 0, -3).Format(time.DateTime)
	var deleteModel = []models.Tabler{
		models.RecordGameRound{},
		models.RecordGameTotal{},
		models.RecordGameCost{},
		models.RecordGameReplay{},
		models.RecordGameDay{},
		models.UserLoginRecord{},
		models.GameOnline{},
		models.HouseActivityRecordLog{},
		models.UserWealthCost{},
		models.HouseLog{},
	}
	for _, mod := range deleteModel {
		go func(deleteMod models.Tabler) {
			if err := GetDBMgr().GetDBmControl().Exec(fmt.Sprintf("delete from %s where created_at < ?", deleteMod.TableName()), timeLimit).Error; err != nil {
				xlog.Logger().Errorf("DeleteOldData %s error: %v", deleteMod.TableName(), err)
			}
		}(mod)
	}

	if GetDBMgr().PHPDB != nil {
		if err := GetDBMgr().PHPDB.Exec("delete from js_daydata where ctime < ?", timeLimit).Error; err != nil {
			xlog.Logger().Errorf("delete js_daydata error: %v", err)
		}

		if err := GetDBMgr().PHPDB.Exec("delete from js_transfer_log where created_at < ?", timeLimit).Error; err != nil {
			xlog.Logger().Errorf("delete js_transfer_log error: %v", err)
		}

		if err := GetDBMgr().PHPDB.Exec("delete from js_script_wanfa where ctime < ?", timeLimit).Error; err != nil {
			xlog.Logger().Errorf("delete js_script_wanfa error: %v", err)
		}

		if err := GetDBMgr().PHPDB.Exec("delete from js_script_house where ctime < ?", timeLimit).Error; err != nil {
			xlog.Logger().Errorf("delete js_script_house error: %v", err)
		}
	}
}
