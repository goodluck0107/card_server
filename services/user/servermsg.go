	...
	case consts.MsgTypeLoginMobile:
		{ // 手机登录(手机号+验证码)
			...
			if err != nil {
				...
				w.Write(getErrReturn(msg.Header, xerrors.NewXError("loginMobile can not creater person err ")))
			} else {
				if person == nil {
					// 提示绑定微信
...
func getLoginResp(msgType string, person *static.Person, isNew bool) (*static.Msg_Header, *xerrors.XError) {
	...
	result.Hall = GetServer().Con.Center
	if GetServer().Con.CenterProxy != "" && GetServer().Con.CenterProxy != "-" {
		result.Hall = GetServer().Con.CenterProxy
	}
	result.Gate = GetServer().Con.Gate
