...
func (sp *SportJZCXZ) sendGameSceneStatusPlay(player *components2.Player) bool {
	...
	sp.OnWriteGameRecord(player.GetChairID(), fmt.Sprintf("StatusPlay.Begin.ActionMask = %d, actions = %v", StatusPlay.ActionMask, getActions(StatusPlay.ActionMask)))

	if player.Ctx.Response {
		sp.OnWriteGameRecord(player.GetChairID(), ">> StatusPlay.ActionMask Response")
		StatusPlay.ActionMask = static.WIK_NULL
	}

	if (StatusPlay.ActionMask & static.WIK_QIANG) != 0 {
		sp.OnWriteGameRecord(player.GetChairID(), ">> StatusPlay.ActionMask WIK_QIANG")
	...
	sp.OnWriteGameRecord(player.GetChairID(), fmt.Sprintf("StatusPlay.End.ActionMask = %d, actions = %v", StatusPlay.ActionMask, getActions(StatusPlay.ActionMask)))
...
func getActions(mask byte) []string {
	var actions []string
	if (mask & static.WIK_QIANG) != 0 {
		actions = append(actions, "WIK_QIANG")
	}
	if (mask & static.WIK_PENG) != 0 {
		actions = append(actions, "WIK_PENG")
	}
	if (mask & static.WIK_GANG) != 0 {
		actions = append(actions, "WIK_GANG")
	}
	if (mask & static.WIK_CHI_HU) != 0 {
		actions = append(actions, "WIK_CHI_HU")
	}
	return actions
}
