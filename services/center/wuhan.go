...
func (s *Server) AddOneGameServer(idGame int, gameserver *static.Msg_GameServer) {
	if s.Con.GameProxy != "" && s.Con.GameProxy != "-" {
		// 如果设置了游戏服代理，则用游戏服务器的
		gameserver.ExIp = s.Con.GameProxy
		gameserver.SafeIp = s.Con.GameProxy
	}
	s.GameLock.CustomLock()
...
*/
func updateWealth(uid int64, wealthType int8, cost int, costType int8) error {
	switch wealthType {
	...
	case consts.WealthTypeCoupon: // 礼券
		tx := GetDBMgr().db_M.Begin()
		_, afterNum, err := wealthtalk.UpdateCoupon(uid, cost, costType, tx)
		if err != nil {
			xlog.Logger().Error(err)
			tx.Rollback()
			return err
		...
		// 更新用户信息
		GetDBMgr().db_R.UpdatePersonAttrs(uid, "GoldBean", afterNum)
		// 通知客户端
		hp := GetPlayerMgr().GetPlayer(uid)
		if hp != nil {
			hp.Info.GoldBean = afterNum
			hp.UpdCoupon(costType)
		}
	case consts.WealthTypeDiamond: // 钻石
		tx := GetDBMgr().db_M.Begin()
		_, afterNum, err := wealthtalk.UpdateDiamond(uid, cost, costType, tx)
		if err != nil {
			xlog.Logger().Error(err)
			tx.Rollback()
			return err
		...
		// 更新用户信息
		GetDBMgr().db_R.UpdatePersonAttrs(uid, "Diamond", afterNum)
		// 通知客户端
		hp := GetPlayerMgr().GetPlayer(uid)
		if hp != nil {
			hp.Info.GoldBean = afterNum
